...
# handles the actual proxy server functionality.
class ProxyWorker(QThread):
    ...
    def __init__(self, host, port): #Initializes the proxy worker with the host and port to listen
        super().__init__()          #constructor of the parent class QThread
        self.host = host
        self.port = port
        self.running = False
        self.active_connections = 0
        self.total_requests = 0
        self.BUFFER_SIZE = 8192 #8kb

    def run(self): # accepts incoming connections
        """Main server loop"""
        server_socket = socket.socket(socket.AF_INET, socket.SOCK_STREAM)
        server_socket.setsockopt(socket.SOL_SOCKET, socket.SO_REUSEADDR, 1)
        
        try:
            server_socket.bind((self.host, self.port)) # assign a port and a ip
            server_socket.listen(10) # listening for connections, with a backlog of 10
            self.running = True
            self.logUpdate.emit(f"Server started on {self.host}:{self.port}") #Qt signal PyQt for GUI

            while self.running: #Accepting Incoming Connections in a Loop
                try:
                    client_socket, client_address = server_socket.accept()
                    self.active_connections += 1
                    self.total_requests += 1
                    self.connectionUpdate.emit(self.active_connections, self.total_requests) #Qt signal PyQt for GUI
                    ...
                    thread.daemon = True #Daemon threads automatically stop when the main program exits
                    thread.start()
                    
                except Exception as e: #Handles errors while the server is running
                    if self.running:
                        self.logUpdate.emit(f"Error accepting connection: {e}")
                    
        except Exception as e: # Handles errors before the server starts.
            self.logUpdate.emit(f"Server error: {e}")
        finally:
            server_socket.close()

    def handle_client(self, client_socket, client_address): #Handles a client connection  determining whether it's an HTTPS or HTTP request
        """Handle client connection"""
        try:
            # Receive initial request
            request_data = client_socket.recv(self.BUFFER_SIZE)
            if not request_data: # closes idle connections
                return
    ...
    def handle_https_tunnel(self, client_socket, client_address, first_line):
        ...
        try:
            ...
            self.connectionAdded.emit([     #Qt signal PyQt for GUI
                timestamp,
                client_address[0],  # Client IP
                host,               # Target Host
            ...
            self.tunnel_traffic(client_socket, server_socket) # TCP tunneling

        except Exception as e:
            self.logUpdate.emit(f"HTTPS tunneling error: {e}")
        finally:
            client_socket.close() # WHY? - Prevents memory leaks

    def tunnel_traffic(self, client_socket, server_socket):
        """Handle bidirectional tunneling between client and server"""

        #source → The socket we are reading data from.
        # destination → The socket we are writing data to.
        # direction → Just a label for logging
    ...
    def handle_http_request(self, client_socket, client_address, request_data):
        ...
        try:
            # Extract URL
            first_line = request_data.decode('utf-8').split('\n')[0] # Decode the first line
            url = first_line.split(' ')[1]
            
            # Extract target host
            http_pos = url.find("://")
            if http_pos == -1:  # If there is no "http://
                temp = url
            else:
                temp = url[(http_pos + 3):] # Remove "http://"
